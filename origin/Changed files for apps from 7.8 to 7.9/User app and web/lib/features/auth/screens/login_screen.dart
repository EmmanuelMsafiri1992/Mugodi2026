...
class _LoginScreenState extends State<LoginScreen> {
                    ...
                    builder: (context, authProvider, child) => Form(key: _formKeyLogin, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Center(child: Padding(
                        ...
                        child: Row(mainAxisSize: MainAxisSize.min, children: [
                          Image.asset(
                            Images.appLogo, height:  40,
                            fit: BoxFit.scaleDown,
                          ),
                          const SizedBox(width: Dimensions.paddingSizeSmall),

                          Text(
                              getTranslated(Provider.of<SplashProvider>(context, listen: false).configModel?.ecommerceName, context),
                              style: poppinsBold.copyWith(color: Theme.of(context).primaryColor, fontSize: 30)
                          )
                        ]),
                      )),
                      const SizedBox(height: Dimensions.paddingSizeExtraSmall),

                      Center(child: Text(getTranslated('welcome_back_!', context), style: poppinsSemiBold.copyWith(fontSize: 22))),
  ...
  void _initLoading() {
    ...
    if(userData != null && userData.loginType == FromPage.login.name) {
      ...
      countryCode ??= CountryCode.fromCountryCode(configModel?.country?? "BD").dialCode;
    }
  }
